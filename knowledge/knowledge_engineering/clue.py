...
class Symbol:
    ...
    def __eq__(self, other):
        return isinstance(other, Symbol) and self.name == other.name  # Check if symbols are equal

    def __hash__(self):
        return hash(self.name)  # Return hash of symbol name
    ...
    def __modelcheck__(self, model):
        ...
        :rtype: bool
        """
        return model[self.name]  # Return symbol value from model
...
class KnowledgeEngineering:
    ...
    def __init__(self, people, rooms, weapons):
        ...
        self.knowledge: set[object] = set()  # Knowledge base as set

        # There must be a person, room, and weapon.
        self.knowledge.add(And(
            Or(self.people[0], self.people[1], self.people[2]),  # One person must be guilty
            Or(self.rooms[0], self.rooms[1], self.rooms[2]),  # One room must be the crime scene
            Or(self.weapons[0], self.weapons[1], self.weapons[2])  # One weapon must be used
        ))
    ...
    def my_card(self, card, i):
        ...
        """
        # Initial cards
        self.knowledge.add(Not(Symbol(card[i])))  # Add negation of my card to knowledge
    ...
    def known_card(self, card):
        ...
        """
        # Known card
        self.knowledge.add(Not(Symbol(card[0])))  # Add negation of first card
        self.knowledge.add(Not(Symbol(card[1])))  # Add negation of second card
        self.knowledge.add(Not(Symbol(card[2])))  # Add negation of third card
    ...
    def guess(self, card):
        ...
        """
        # Unknown card
        self.knowledge.add(Or(Not(Symbol(card[0])), Not(Symbol(card[1])),
                              Not(Symbol(card[2]))))  # At least one card is not the solution

    def check_guess(self):
        """
        Check if any symbol can be determined as definitely true or false

        :return: result of the guess check
        :rtype: str
        """
        kb = list(self.knowledge)  # Convert knowledge set to list
        propositions = [symbol.name for symbol in self.symbols]  # Get all symbol names

        for symbol in self.symbols:  # Check each symbol
            # Check if symbol is entailed (true in all valid models)
            mc_pos = ModelCheck(kb, symbol.name, {}, propositions)
            valid_models = mc_pos.model_mapping()

            if valid_models and all(model[symbol.name] for model in valid_models):
                return f"{symbol.name} is YES"

            # Check if negation is entailed (false in all valid models)
            if valid_models and all(not model[symbol.name] for model in valid_models):
                return f"{symbol.name} is NO"
...
def main():
    ...
    game1.my_card(("mustard", "kitchen", "revolver"), 0)
    game1.my_card(("mustard", "kitchen", "revolver"), 1)
    game1.my_card(("mustard", "kitchen", "revolver"), 2)

    result = game1.check_guess()
    print(result)
